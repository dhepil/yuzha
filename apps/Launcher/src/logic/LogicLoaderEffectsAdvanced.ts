...
function canUseAdvanced(): boolean {
  // Gate heavy effects on capability
  // WebGL presence + light device heuristic
...
export function buildEffectsAdvanced(app: Application, built: BuiltLayer[]) {
  ...
  for (let i = 0; i < built.length; i++) {
    ...
    for (const e of fx) {
      ...
      if ((e as any).type === 'glow') {
        ...
        auraSprite.blendMode = 1 // ADD
        b.sprite.parent?.addChildAt(auraSprite, b.sprite.parent.getChildIndex(b.sprite))
